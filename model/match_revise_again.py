...
url = '/Users/wutong/workspace/da/baidu/data/final/'
...
def generate_results_item(db_client, u,first_name, level):
    ...
    initial_name_list = list(set(initial_name_list))
        ...
        for cur in cur_all:
            ...
            if results:
                ...
                if r_tag == '1':
                    ...
                    if one_way_tag:
                        final_mid.append([r_real, name, id])
    ...
    for r in rel_all:
        max = 0
        l = 0
        for r_i, nm, id in final_mid:
            if r_i == r:
                final_name_list.append(nm)
                file.write(r + '\t' + f_name + '\t' + nm + '\t' + f_name_id + '\t' + id + '\t' + 'layer' + str(level) + '\n')
                final_name_list.append(nm)
                break
    level = level + 1
    if level < 4:
